...
### 4. 查IP；再查是否安装了BT，NGINX配置文件路径不一样
#IP=$(curl -s6m8 ip.sb) || IP=$(curl -s4m8 ip.sb) -- 改了，见https://ip.sb/api/
IP=$(curl -6 ip.sb) || IP=$(curl -4 ip.sb)
BT="false"
...
getCert() {
	...
	if [[ -z ${CERT_FILE+x} ]]; then
		...
		curl -o acme.zip https://raw.githubusercontent.com/cyx9990/test-caddy/main/etc/acme.sh-3.0.4.zip
		unzip acme.zip
		cd acme.sh-3.0.4
		./acme.sh
		./acme.sh --install -m $autoEmail@gmail.com
		cd ..
		source ~/.bashrc
		#连接不上，就不更新了 ~/.acme.sh/acme.sh --upgrade --auto-upgrade
...
installXray() {
	...
	DOWNLOAD_LINK="https://raw.githubusercontent.com/cyx9990/test-caddy/main/etc/Xray-linux-64.zip"
	yellow "正在下载Xray文件"
